...
/**
 * Watch for new translations in the server side.
 *
 * @author Joseluis Laso <jlaso@joseluislaso.es>
 */
class TranslationsWatchCommand extends ContainerAwareCommand
{
    /** @var InputInterface */
    private $input;
    /** @var OutputInterface */
    ...
    {
        $this->setName('jlaso:translations:watch');
        $this->setDescription('Watch for new translations in server side.');
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->init($input->getOption('address'), $input->getOption('port') ?: self::DEFAULT_PORT);

        $config         = $this->getContainer()->getParameter('translations_api');
        $managedLocales = $config['managed_locales'];

        $this->output->writeln(PHP_EOL . '<info>*** Watching for new translations ***</info>');

        $conn = $this->clientApiService->register();
        var_dump($conn);
